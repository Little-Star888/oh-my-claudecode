...
function normalizeRawTeamEvent(raw) {
    ...
    if (!eventId || !team || !type || !worker || !createdAt)
        return null;
    if (type === 'worker_idle') {
        return {
            ...value,
    ...
    return {
        ...
        message_id: typeof value.message_id === 'string' || value.message_id === null ? value.message_id : undefined,
...
function isDuplicateNormalizedEvent(previous, current) {
    if (!previous)
        return false;
    if (previous.type !== 'worker_state_changed' || current.type !== 'worker_state_changed')
        return false;
...
function matchesEventType(event, type) {
    if (!type)
        return true;
    if (event.type === type)
        return true;
...
function matchesEventQuery(event, opts) {
    if (!matchesEventType(event, opts.type))
        return false;
...
export async function readTeamEvents(teamName, cwd, opts = {}) {
    ...
    for (const line of raw.split('\n')) {
        ...
        if (isDuplicateNormalizedEvent(previous, normalized))
            continue;
        previous = normalized;
        if (opts.wakeableOnly && !isWakeableTeamEventType(normalized.type))
            continue;
        if (!matchesEventQuery(normalized, opts))
            continue;
...
export async function waitForTeamEvent(teamName, cwd, opts) {

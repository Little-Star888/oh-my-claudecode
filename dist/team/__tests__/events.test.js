...
async function setupTeam(name) {
    const cwd = await mkdtemp(join(tmpdir(), `omc-team-events-${name}-`));
    await initTeamState(teamConfig(name), cwd);
...
describe('team/state/events', () => {
    ...
    it('treats integration requests, merge conflicts, and stale alerts as wakeable while keeping audit-only events filtered', async () => {
        ...
        try {
            ...
            assert.deepEqual(wakeable.map((event) => event.type), ['worker_merge_conflict', 'worker_cherry_pick_conflict', 'worker_rebase_conflict', 'worker_integration_attempt_requested', 'worker_cross_rebase_conflict', 'worker_stale_stdout']);
